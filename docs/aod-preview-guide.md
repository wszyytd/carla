...
已有 v1 数据的用户请先看 [v2 尺度校准与新旧对比](aod-scale-calibration.md)。新配置默认使用 60/100/140 m、轴向偏移 60 m；旧配置缺少 view_grid 时仍执行原来的 20/30/40 m、±10 m。
...
```bash
python -m src.aod inventory --config cfg/simulator.yaml --output out/aod_inventory_v2.json
python -m src.aod prepare --inventory out/aod_inventory_v2.json --spawn-index 0 --output out/aod_preview_v2.yaml
...
- 默认把出生点 z 当成临时高度基准，标记为 `spawn_point_unverified`；这不是地面测量。知道实际基准高度后，用 prepare 的 `--ground-z` 明确指定。相机高度是基准加 view_grid.heights_m 中的高度。
- 所有输出都拒绝覆盖；改配置或重试时换新的版本文件名和新的采集目录。
...
水平位置依次为相对中心的 `(60,0), (-60,0), (0,60), (0,-60), (60,60), (60,-60), (-60,60), (-60,-60)` 米。对角点更远，这不是只改变方位角的等距离实验。默认 RGB 为 1920×1080、水平 FOV 60°，仿真步长 0.05 s，相机周期 0.1 s。
...
data/aod_preview_v2/
  ...
  images/c00_h60_p1_rgb.png       原 RGB
  images/c00_h60_p1_instance.png  无损实例颜色；未转换为可视化调色板
  images/c00_h60_p1_overlay.png   原图加目标投影框
  images/c00_h60_p1_crop.png      固定 300×300 裁剪
  contact_sheet_00.png            类别 0 的 3×8 预览拼图
  crop_sheet_00.png              原尺度 crop 拼图；不缩放目标
  scale_report.json              投影框像素尺寸与截断统计
